...
endpoint.models.get = function(model, httpOptions) {
    ...
    sys.logs.debug('[chatgpt] GET from: ' + url);
	var options = checkHttpOptions(url, httpOptions);
	return endpoint._get(options);
};

endpoint.completions.post = function(httpOptions) {
    var url = parse('/v1/completions');
    sys.logs.debug('[chatgpt] POST from: ' + url);
    var options = checkHttpOptions(url, httpOptions);
    return endpoint._post(options);
};

endpoint.chat.completions.post = function(httpOptions) {
    var url = parse('/v1/chat/completions');
    sys.logs.debug('[chatgpt] POST from: ' + url);
    var options = checkHttpOptions(url, httpOptions);
    return endpoint._post(options);
};

endpoint.edits.post = function(httpOptions) {
    var url = parse('/v1/edits');
    sys.logs.debug('[chatgpt] POST from: ' + url);
    var options = checkHttpOptions(url, httpOptions);
    return endpoint._post(options);
};
